...
def expt_fig_directionality(procs, dept_vars, fig_name=None, xlim_map=None, ylim_map=None, **kwargs):
    ...
    fig, axs = create_figure(len(procs) * len(dept_vars), nrows=1, fig_name=fig_name, **kwargs)


    for proc, ax in zip(procs, axs[0]):
        # load the experimental data appropriate for the procedure
        actual_summ = get_data(proc)
        if proc == 'BF':
...
def get_data(proc):
    # retrieves group summary data for the specified procedure containing bout direction percentages
    bouts_group_summ_file = "../data/expt/all/bouts_group_summ.csv"
    gdf = load_data(bouts_group_summ_file)
    df = gdf[gdf.source == proc].drop(columns=['source'])
    return df
...
def test():
    ...
    dept_vars = ['forward']
    fig_name = 'forward'
    expt_fig_directionality(procs=['OR', 'BF'], dept_vars=dept_vars, fig_name=fig_name, size=(120, 60))
